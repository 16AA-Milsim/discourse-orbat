...
- Within the `orbat_json` configuration, the `rankPriority` array controls rank ordering (highest first). Members are always ordered by role group first (`Coy_IC`, then `Coy_2IC`, then other `*_IC`, then other `*_2IC`, then regular members), then by `rankPriority`, then by join date, then alphabetically by username. Nodes also accept `marginLeft` and `marginRight` values (negative values reduce spacing) to add extra horizontal spacing.
