...
- `orbat_uniform_hover_preview_enabled` (default `false`) enables uniform PNG hover previews over member names. This is adaptive: if `discourse-project-uniform` is missing or public PNGs are disabled, ORBAT continues normally without previews.
- Within the `orbat_json` configuration, the `rankPriority` array controls rank ordering (highest first). By default, members are ordered by role group first (`Coy_IC`, then `Coy_2IC`, then other `*_IC`, then other `*_2IC`, then regular members), then by `rankPriority`, then by join date, then alphabetically by username. Set `select.sort` to `rank` on a node to use rank/join-date/alphabetical ordering without role grouping. Nodes also accept `marginLeft` and `marginRight` values (negative values reduce spacing) to add extra horizontal spacing.
