...
- Within the `orbat_json` configuration, the `rankPriority` array controls rank ordering (highest first). Members are ordered by the group names listed in each node’s `select.any`/`select.all`, then by `rankPriority`, then by join date, then alphabetically.
